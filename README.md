...
- Селебрити проблем  
   Изменения:  
    1. Добавляем кэш селеб в Redis( selebrity(user_id)), добавляем кэш подпискичов Redis (subscribe_user(user_id, author_id)).
    2. Публикация поста + пример чтения ленты (при наличии кэша) 
       ![selebrity_seq.svg](selebrity_seq.svg)  
- N+1 проблема  - batch гидратация
  При чтании ленты постов нам нужна гидратация. Если отсутвует кэш постов в Redice, то мы идем их запрашивать. Чтобы не образовалась N(количество постов)+1 запрос к бд, мы запрашивает сразу  batch-постов в одном запросе
  ![seq2.svg](seq2.svg) 
- Прогретый кэш 
     Добавляем кластер в Redis(последние посты самых популярных авторов за день hot_posts(post_id, author_id, timestamp)). Обновляем раз в день по расписанию. И делаем реплики. 
  Когда пользователь впервые заходит в соц сеть, показываем ему этот кэш.
...
4. Технические решения (сравнение и выбор)

   **Стратегия формирования ленты**
    * `Fan-in` - формирование ленты при запросе пользователя
    ...
    * `Fan-out` - формирование ленты при записи поста в систему
